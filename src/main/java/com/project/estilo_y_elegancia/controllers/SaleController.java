...
public class SaleController extends BaseController<MdSale, SaleServiceImpl> {
 ...
 @GetMapping("/get/sales/customer/{idCustomer}")
 public ResponseEntity<?> getSalesByCustomer(@PathVariable Long idCustomer) throws Exception {
  try {
   return ResponseEntity.ok(service.getSalesByIdCustomer(idCustomer));
  } catch (Exception e) {
   return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
  }
 }
